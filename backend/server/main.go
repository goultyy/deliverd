...
func StartServer() {
	...
	app.Get("/auth/keys/get_owner/:key_type/:owner_id", h_get_key_by_owner) // Get keys by owner id
	...
	app.Get("/packages/get/status/:status", h_get_packages_by_status) // Get a package by status
	app.Get("/packages/get/id/:id", h_get_package_by_id)              // Get a package by ID
	app.Post("/packages/new", h_new_package)
